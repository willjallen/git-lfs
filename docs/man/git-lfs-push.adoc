...
When used with `lfs.storagecache=false`, successful uploads to remotes
listed in `lfs.knowngoodremote` cause Git LFS to remove the corresponding
objects from the local cache after the push completes.
